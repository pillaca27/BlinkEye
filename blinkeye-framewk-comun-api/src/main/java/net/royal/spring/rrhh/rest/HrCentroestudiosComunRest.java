...
import net.royal.spring.rrhh.dominio.dto.DtoComunHrCentroestudioCarrera;
import net.royal.spring.rrhh.dominio.dto.DtoComunHrCentroestudios;
import net.royal.spring.rrhh.dominio.filtro.FiltroComunHrCentroestudios;
import net.royal.spring.rrhh.dominio.filtro.FiltroComunHrCentroestudiosCarrera;
...
public class HrCentroestudiosComunRest extends GenericoHibernateRest {
	 ...
	 */
	@Transactional
	@PostMapping(value = "/listarPaginacionConCarrera",  produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<DominioPaginacion> listarPaginacionConCarrera(
			@RequestBody FiltroComunHrCentroestudiosCarrera filtro)
    {
		Integer cantidadEncontrado = 0;
        List<DominioParametroPersistencia> parametros = new ArrayList<DominioParametroPersistencia>();
        if (UInteger.esCeroOrNulo(filtro.getCentro()))
        ...
        if (UString.estaVacio(filtro.getDescripcion()))
            filtro.setDescripcion(null);
        if (UString.estaVacio(filtro.getRegimen()))
            filtro.setRegimen(null);
        if (UString.estaVacio(filtro.getTipoinstitucion()))
            filtro.setTipoinstitucion(null);
        
        parametros.add(new DominioParametroPersistencia("p_centro", Integer.class, filtro.getCentro()));
        parametros.add(new DominioParametroPersistencia("p_descripcion", String.class, filtro.getDescripcion()));
        parametros.add(new DominioParametroPersistencia("p_regimen", String.class, filtro.getRegimen())); 
        parametros.add(new DominioParametroPersistencia("p_tipo", String.class, filtro.getTipoinstitucion()));
        
        cantidadEncontrado = this.contar("hrcentroestudios.listarPaginacionCarreraContar", parametros);
        List lstRegistros = this.listarConPaginacion(filtro.getPaginacion(), parametros, "hrcentroestudios.listarPaginacionCarreraSql", 
        		DtoComunHrCentroestudioCarrera.class);
	 ...
	 */
	@Transactional
	@PostMapping(value = "/listarCarreraPorCentroEstudio",  produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<DtoComunHrCentroestudioCarrera>> listarCarreraPorCentroEstudio(
			@RequestBody DtoComunHrCentroestudioCarrera dto)
    {
		List<DtoComunHrCentroestudioCarrera> lstRegistros = new ArrayList<DtoComunHrCentroestudioCarrera>();
        List<DominioParametroPersistencia> parametros = new ArrayList<DominioParametroPersistencia>();
        
        parametros.add(new DominioParametroPersistencia("p_centro", Integer.class, dto.getCentro()));

        List lst = this.listarPorQuery(DtoComunHrCentroestudioCarrera.class, 
        		"hrcentroestudios.listarCarrerasPorCentro", parametros);
        if (lst.size() > 0)
        	lstRegistros = (List<DtoComunHrCentroestudioCarrera>)lst;
        
        return new ResponseEntity<List<DtoComunHrCentroestudioCarrera>>(lstRegistros, HttpStatus.OK);
    }

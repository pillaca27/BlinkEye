...
@RestController
@RequestMapping("/spring/rrhh/hrafpcomun")
@CrossOrigin(origins = "*")
public class HrAfpComunRest extends GenericoHibernateRest {

	private static Logger logger = LogManager.getLogger(HrAfpComunRest.class);
	...
	@ApiOperation(notes = "-Descripcion: Listado Activos Tipo trabajador de la tabla Hr_Tipotrabajador y estado activo | Sin parametros de entrada | Parametros de salida: codigo, nombre", 
			value = "HR-AFP-LISTAR", tags = {"RRHH", "AFP"})
	@Transactional
	@GetMapping(value = "/listar", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<DtoTabla>> listar() {
		logger.debug("listar Activos Tipo Trabajador");
		List datos = this.listarPorQuery(DtoTabla.class, "hrafp.listar");
	 ...
	 */
	@ApiOperation(notes = "-Descripcion: Listado Activos Tipo trabajador de la tabla Hr_Tipotrabajador y estado activo | Sin parametros de entrada | Parametros de salida: codigo, nombre", 
			value = "HR-AFP-LISACT", tags = {"RRHH", "AFP"})
	@Transactional
	@GetMapping(value = "/listaractivos", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<DtoTabla>> listarActivos() {
		logger.debug("listar Activos Tipo Trabajador");
		List datos = this.listarPorQuery(DtoTabla.class, "hrafp.listarActivos");
		return new ResponseEntity<List<DtoTabla>>(datos, HttpStatus.OK);
	}
	...
	public ResponseEntity<List<DtoTabla>> listarFiltros(@RequestBody DtoTabla filtro) {
		...
		parametros.add(new DominioParametroPersistencia("p_codigoafp", String.class, filtro.getCodigo()));
		parametros.add(new DominioParametroPersistencia("p_nombreafp", String.class, filtro.getNombre()));
		parametros.add(new DominioParametroPersistencia("p_estado", String.class, filtro.getEstadoId()));
		List datos = this.listarPorQuery(DtoTabla.class, "hrafp.listarfiltros",parametros);
		return new ResponseEntity<List<DtoTabla>>(datos, HttpStatus.OK);
	}
	...
	public ResponseEntity<DtoTabla> obtenerTabla(@RequestBody DtoTabla filtro) { 
		List<DominioParametroPersistencia> parametros = new ArrayList<DominioParametroPersistencia>();
        parametros.add(new DominioParametroPersistencia("p_codigoafp", String.class, filtro.getCodigo()));
        List datos = this.listarPorQuery(DtoTabla.class, "hrafp.obtenertabla", parametros);
        DtoTabla dto = null;
        if (datos.size()==1)
	 ...
	 */
	@Transactional
	@PostMapping(value = "/listarafpporfiltro", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<DominioPaginacion> listarAfpPorFiltro(@RequestBody FiltroComunHrAfp filtro)
    {
        logger.debug("TipopagoRest.listarafpporfiltro");        
        Integer cantidadEncontrados = 0;
        List<DominioParametroPersistencia> parametros = new ArrayList<DominioParametroPersistencia>();
        
        if(UString.estaVacio(filtro.getAfp()))
        	filtro.setAfp(null);
        if(UString.estaVacio(filtro.getNombre()))
        	filtro.setNombre(null);

        parametros.add(new DominioParametroPersistencia("p_afp", String.class, filtro.getAfp()));
        ...
        cantidadEncontrados = this.contar("hrafp.contarafpporfiltro", parametros);

        List lstResultado = this.listarConPaginacion(filtro.getPaginacion(), parametros, "hrafp.listarafpporfiltro", 
        		DtoTabla.class);

...
public class CompanyownerrecursoComunRest extends GenericoHibernateRest {
	...
	/**
	 * FALTA no se implemento en angular aun / es necesario ????
	 * 
	 * @param dto
	 * @return
	 */
	...
	public String obtenerImagenComoCadena(@RequestBody CompanyownerrecursoTransaccion dto) {
		String urlImagen = "";
		...
		if (tipoImagen == ConstanteDatos.TIPO_IMAGEN.LOGO) {
			...
			pk.setPeriodo(UString.esNuloVacioTrim(periodo) ? "999999" : periodo);
			pk.setTiporecurso(tipoReporte); //ConstanteDatos.CODIGO_IMAGEN_COMPANIA
			...
			if (recurso != null) base64 = "data:image/png;base64," + Base64.getEncoder().encodeToString(recurso.Recurso);
            urlImagen = base64;
		}
...

		{
			...
			// byte[] decoded = Base64Utils.decode(recurso.Recurso);
			if (recurso != null)
				base64 = "data:image/png;base64," + Base64.getEncoder().encodeToString(recurso.Recurso);
		...
		if (tipoImagen == ConstanteDatos.TIPO_IMAGEN.IMAGEN) {
			...
			// byte[] decoded = Base64Utils.decode(recurso.Recurso);
			if (recurso != null)
				base64 = Base64.getEncoder().encodeToString(recurso.Recurso);
	...
	@Transactional
	@PutMapping(value = "/obtenerRecurso", produces = MediaType.APPLICATION_JSON_VALUE)
	public BeanCompanyownerrecurso obtenerRecurso(@RequestBody CompanyownerrecursoTransaccion dto) {
		String compania = dto.getCompania();
		ConstanteDatos.TIPO_IMAGEN tipoImagen = dto.getTipoImagen();
		String periodo = dto.getPeriodo();
		String tipoReporte = dto.getTipoReporte();
		
		BeanCompanyownerrecurso recurso =new BeanCompanyownerrecurso();
	...
	public ResponseEntity<DtoComunCompanyownerrecurso> registrar(@RequestBody DtoComunCompanyownerrecurso dto)
			throws Exception {
		BeanCompanyownerrecurso bean = new BeanCompanyownerrecurso();
		bean.getPk().setCompanyowner(dto.getCompanyowner());
		bean.getPk().setPeriodo(dto.getPeriodo());
		...
		if (bean.getAuxString() != null && bean.getAuxString() != "") {
			String bin = bean.getAuxString().substring(bean.getAuxString().indexOf(",") + 1);
			bean.setRecurso(Base64.getDecoder().decode(bin));
...
//			bean.setRecurso(name);
	...
	public ResponseEntity<DtoComunCompanyownerrecurso> actualizar(@RequestBody DtoComunCompanyownerrecurso dto)
			throws Exception {
		BeanCompanyownerrecurso bean = new BeanCompanyownerrecurso();
		bean.getPk().setCompanyowner(dto.getCompanyowner());
		bean.getPk().setPeriodo(dto.getPeriodo());

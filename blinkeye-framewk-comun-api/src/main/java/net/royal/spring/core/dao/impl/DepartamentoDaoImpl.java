...
public class DepartamentoDaoImpl extends GenericoDaoImpl<BeanDepartamento, BeanDepartamentoPk> {
	...
	public DtoComunDepartamento obtenerDtoPorUuid(DtoComunDepartamento pk) {
		DtoComunDepartamento dto = null;
		List<DominioParametroPersistencia> parametros = new ArrayList<DominioParametroPersistencia>();
		parametros.add(new DominioParametroPersistencia("p_uuid", String.class, pk.getUuid()));
		List lst = listarPorQuery(DtoComunDepartamento.class, "departamento.obtenerDtoPorUuid", parametros);
		if (lst.size() == 1) {

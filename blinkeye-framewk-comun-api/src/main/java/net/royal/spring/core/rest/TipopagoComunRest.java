...
/*MANTENIMIENTO LEONARDO*/
	...
	public ResponseEntity<DtoComunTipopago> obtenerDto(@RequestBody DtoComunTipopago pk) throws Exception {
		...
		DtoComunTipopago dto = tipopagoDao.obtenerDtoPorUuid(pk.getUuid());
		return new ResponseEntity<DtoComunTipopago>(dto,HttpStatus.OK);
	}
	...
	/**
	 * MALCAN VALIDADO
	 * TIPO PAGO COMUN SELECTOR
	 */
	@Transactional
	@PostMapping(value = "/listartipopagoporfiltro", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<DominioPaginacion> listarTipoPagoPorFiltro(@RequestBody FiltroComunTipopago filtro)
    {
        logger.debug("TipopagoRest.listartipopagoporfiltro");        
        Integer cantidadEncontrados = 0;
        List<DominioParametroPersistencia> parametros = new ArrayList<DominioParametroPersistencia>();
        
        if(UString.estaVacio(filtro.getTipopago()))
        	filtro.setTipopago(null);
        ...
        cantidadEncontrados = this.contar("tipopago.contartipopagoporfiltro", parametros);

        List lstResultado = this.listarConPaginacion(filtro.getPaginacion(), parametros, "tipopago.listartipopagoporfiltro", 
        		DtoComunTipopago.class);
	...
	public void exportarTipoPago(HttpServletResponse response, @RequestBody FiltroComunTipopago filtro) throws Exception
	 {
		...
		DominioPaginacion paginacion = tipopagoDao.listarPaginado(this.getUsuarioActual(), filtro); //tipopagoDao.exportarTipoPago(filtro);

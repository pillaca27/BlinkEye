...
public class PaisDaoImpl extends GenericoDaoImpl<BeanPais, BeanPaisPk> {
	...
	public DtoComunPais obtenerDtoPorUuid(String uuid) {
		DtoComunPais dto = null;
		List<DominioParametroPersistencia> parametros = new ArrayList<DominioParametroPersistencia>();
		parametros.add(new DominioParametroPersistencia("p_uuid", String.class, uuid));
		List lst = listarPorQuery(DtoComunPais.class, "maunidadnegocio.obtenerDtoPorUuid", parametros);
		if (lst.size() == 1) {
	...
	public List<DtoComunPais> listarDtoCore(DtoComunPais filtro) throws Exception {
		...
		// TODO BeanPais.listarDtoCore : modificar query / modificar propiedades
	...
	public DominioPaginacion listarPaginado(SeguridadUsuarioActual usuarioActual, FiltroComunPais filtro) {
		...
		// TODO BeanPais.listarPaginado : modificar query / modificar propiedades
		if (UString.esNuloVacio(filtro.getPais()))
			filtro.setPais(null);
		if (UString.esNuloVacio(filtro.getEstado()))
			filtro.setEstado(null);

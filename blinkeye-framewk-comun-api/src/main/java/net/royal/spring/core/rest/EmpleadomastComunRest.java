...
public class EmpleadomastComunRest extends GenericoHibernateRest {
	...
	@Transactional
	@GetMapping(value = "/verificarEmpleadoExiste", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<EmpleadomastTransaccion> VerificarEmpleadoExiste(@RequestBody FiltroComunEmpleado filtro) {
		...
		List datos = this.listarPorQuery(EmpleadomastTransaccion.class, "empleadomast.VerificarEmpleadoExiste", parametros);
		EmpleadomastTransaccion empleadomastTransaccion = (EmpleadomastTransaccion)datos.get(0);
		return new ResponseEntity<EmpleadomastTransaccion>(empleadomastTransaccion, HttpStatus.OK);
	}
...
//	}
	...
	public ResponseEntity<DtoComunEmpleadomast> obtenerDto(@RequestBody DtoComunEmpleadomast pk) throws Exception {
		...
		DtoComunEmpleadomast dto = empleadomastDao.obtenerDtoCore(this.getUsuarioActual(),pk);
		return new ResponseEntity<DtoComunEmpleadomast>(dto,HttpStatus.OK);
	}
	...
	/*
	 * MALCA
	 * LISTADO EVALUADOS PAGINADO 
	 * */
	@Transactional
	@PutMapping(value="/listarEvalPaginado", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<DominioPaginacion> listarEvalPaginado(@RequestBody FiltroComunEmpleadomast filtro) throws Exception {
		logger.debug("EmpleadomastRest.listarEvalPaginado");
		DominioPaginacion paginacion = empleadomastDao.listarEvalPaginado(this.getUsuarioActual(), filtro);
		return new ResponseEntity<DominioPaginacion>(paginacion, HttpStatus.OK);
	}
	
	@Transactional
	@GetMapping(value="/obtenerLstCompanias", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<DtoTabla>> obtenerLstCompanias() throws Exception {
		logger.debug("EmpleadomastRest.obtenerLstCompanias");
		return new ResponseEntity<List<DtoTabla>>(empleadomastDao.obtenerLstCompanias(), HttpStatus.OK);
	}

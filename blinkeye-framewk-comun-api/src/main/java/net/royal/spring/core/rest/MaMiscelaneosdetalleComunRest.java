...
public class MaMiscelaneosdetalleComunRest extends GenericoHibernateRest {
	...
	public ResponseEntity<List<DtoTabla>> listartablaporheader(
		...
		if (UString.esNuloVacio(dto.getCodigoelemento()))
			dto.setCodigoelemento(null);		
		if (UString.esNuloVacio(dto.getDescripcionlocal()))
			dto.setDescripcionlocal(null);	
		...
		parametros.add(new DominioParametroPersistencia("p_codigoelemento", String.class, dto.getCodigoelemento()));
		parametros.add(new DominioParametroPersistencia("p_descripcion", String.class, dto.getDescripcionlocal()));
	 ...
	 * MALCA IMPLEMENTADO
	 * @param pk
	 * @return Lista de Miscelaneos sin orden especificado
	 * @throws Exception
	 */
	@Transactional
	@PutMapping(value="/listarDtoPorAplicacionCodigo", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<List<DtoComunMaMiscelaneosdetalle>> listarDtoPorAplicacionCodigo(@RequestBody DtoComunMaMiscelaneosdetalle filtro) throws Exception {
		logger.debug("listarDtoPorAplicacionCodigo");
		filtro.setEstado("A");
		List<DtoComunMaMiscelaneosdetalle> listado = maMiscelaneosdetalleDao.listarDtoPorAplicacionCodigo(filtro);
		return new ResponseEntity<List<DtoComunMaMiscelaneosdetalle>>(listado,HttpStatus.OK);
	}
}

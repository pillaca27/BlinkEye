	...
	public ReporteTransaccion obtenerConfiguracionReporte(ReporteTransaccion reporte) throws Exception {
		...
		if (reporte != null) {
			idReporteC = "app=" + UString.obtenerSinNulo(reporte.getAplicacionCodigo()) + "|codigo="
					+ UString.obtenerSinNulo(reporte.getReporteCodigo())
					+ "|compania="+UString.obtenerSinNulo(reporte.getCompaniaSocio())
					+ "|periodo="+UString.obtenerSinNulo(reporte.getPeriodo())
					+ "|version="+UString.obtenerSinNulo(reporte.getVersion());
		}
		...
		BeanSyReporte syReporte = syReporteDao.obtenerPorId(new BeanSyReportePk(reporte.getAplicacionCodigo(), reporte.getReporteCodigo()));
		if (syReporte == null) {
			reporte.setTransaccionEstado(DominioTransaccion.ERROR);
			reporte.getTransaccionListaMensajes().add(new DominioMensajeUsuario(tipo_mensaje.ERROR, idReporteC + ":Reporte no encontrado"));
			logger.error(idReporteC + ":Reporte no encontrado");
			this.ErrorRegistrar(new ErrorTransaccion("SY_REPORTE",idReporteC + ":Reporte no encontrado","SyReporteMotorServicioImpl"));			
			return reporte;
		}			
		if (UString.estaVacio(syReporte.getTiporeporte())) {
			reporte.setTransaccionEstado(DominioTransaccion.ERROR);
			reporte.getTransaccionListaMensajes().add(new DominioMensajeUsuario(tipo_mensaje.ERROR, idReporteC + ":Reporte no tiene especificado un tipo"));
			logger.error(idReporteC + ":Reporte no tiene especificado un tipo");
			this.ErrorRegistrar(new ErrorTransaccion("SY_REPORTE",idReporteC + ":Reporte no tiene especificado un tipo","SyReporteMotorServicioImpl"));
			return reporte;
		}			
		if (UString.estaVacio(rutaReportes)) {
			reporte.setTransaccionEstado(DominioTransaccion.ERROR);
			reporte.getTransaccionListaMensajes().add(new DominioMensajeUsuario(tipo_mensaje.ERROR, idReporteC + ":No existe una ruta en donde se encuentren los reportes fisicos"));
			logger.error(idReporteC + ":No existe una ruta en donde se encuentren los reportes fisicos");
			this.ErrorRegistrar(new ErrorTransaccion("SY_REPORTE",idReporteC + ":No existe una ruta en donde se encuentren los reportes fisicos","SyReporteMotorServicioImpl"));
			return reporte;
		}
		...
		if (reporte.getOrigen().equals(ReporteTransaccion.ORIGEN_WORKFLOW) && reporte.getWorkFlowFlgCorreoNiveles().equals("S") ) {
			...
			/*syReportearchivo = syReportearchivoDaoImpl
					.obtenerPorId(new BeanSyReportearchivoPk(reporte.getAplicacionCodigo(), reporte.getReporteCodigo(),
							reporte.getCompaniaSocio(), reporte.getPeriodo(), reporte.getWorkFlowEstado()));*/
			// Version(accionWf.substring(0, 6)
			
			// como en version ya se manda la ACCION DE WORKFLOW, se tomara el campo de periodo 
		...
		if (syReportearchivo == null) {
			reporte.setTransaccionEstado(DominioTransaccion.ERROR);
			reporte.getTransaccionListaMensajes().add(new DominioMensajeUsuario(tipo_mensaje.ERROR, idReporteC + ":Reporte no tiene ningun Archivo"));
			logger.error(idReporteC + ":Reporte no tiene ningun Archivo");
			this.ErrorRegistrar(new ErrorTransaccion("SY_REPORTE",idReporteC + ":Reporte no tiene ningun Archivo","SyReporteMotorServicioImpl"));
			return reporte;
		}

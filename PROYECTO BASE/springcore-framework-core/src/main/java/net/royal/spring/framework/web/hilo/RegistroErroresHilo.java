...
//			logger.debug("RegistroErroresHilo :");
//			logger.debug(apiErrorRegistrar);
//			HttpEntity<ErrorTransaccion> request = new HttpEntity<ErrorTransaccion>(bean, httpHeaders);
//			ResponseEntity<ErrorTransaccion> res = restTemplate.exchange(uri, HttpMethod.POST, request,ErrorTransaccion.class);
//			logger.debug(res.getBody().getTransaccionEstado());
//			logger.debug(res.getBody().getTransaccionListaMensajes().size());			
//		} catch (Exception e) {

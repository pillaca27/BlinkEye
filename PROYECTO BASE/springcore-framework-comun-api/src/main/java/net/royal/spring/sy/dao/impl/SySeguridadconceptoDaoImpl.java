...
import net.royal.spring.sg.dominio.lista.DtoComunSeguridadConceptoList;
...
public class SySeguridadconceptoDaoImpl extends GenericoDaoImpl<BeanSySeguridadconcepto, BeanSySeguridadconceptoPk> {
	...
	public List<DtoComunSySeguridadconcepto> listarPaginadoSeguridadConcepto(SeguridadUsuarioActual usuarioActual, FiltroComunSySeguridadconcepto filtro) {
		List<DominioParametroPersistencia> parametros = new ArrayList<DominioParametroPersistencia>();
		...
		//Integer registros = contar("syseguridadconcepto.listarPaginadoSeguridadConceptoContar", parametros);
		//logger.debug(registros);
		List lst = listarPorQuery( DtoComunSySeguridadconcepto.class, "syseguridadconcepto.listarPaginadoSeguridadConcepto", parametros);
	...
	public DtoComunSeguridadConceptoList actualizarSeguridadConcepto(SeguridadUsuarioActual usuarioActual, DtoComunSeguridadConceptoList dto) {
		
		
		for (DtoComunSySeguridadconcepto e : dto.getListado()) {
			
			if(e.getFlgactualizar().equals("S")){
				BeanSySeguridadconcepto bean = this.obtenerPorId(dto.getFiltro().getAplicacioncodigo(), dto.getFiltro().getGrupo(), e.getConcepto());
				if(bean != null) {
					//ACTUALIZA
					bean.setDescripcionlocal(e.getDescripcionlocal());
					...
					bean.setPk(new BeanSySeguridadconceptoPk(dto.getFiltro().getAplicacioncodigo(), dto.getFiltro().getGrupo(),e.getConcepto()));
					bean.setDescripcionlocal(e.getDescripcionlocal());
					bean.setUltimousuario(usuarioActual.getUsuario());
					bean.setUltimafechamodif(new Date());				
					bean.setEstado("A");
					bean.setOrdenpresentacion( obtenerOrdenpresentacion(bean.getPk().getAplicacioncodigo(), bean.getPk().getGrupo(), bean.getPk().getConcepto())  );
					this.registrar(bean);
				}
			}			
	...
	private Integer obtenerOrdenpresentacion(String aplicacion, String grupo, String concepto) {
		Criteria criteria = getCriteria().add(Restrictions.eq("pk.aplicacioncodigo", aplicacion)).add(Restrictions.eq("pk.grupo", grupo)).add(Restrictions.eq("pk.concepto", concepto))
				.setProjection(Projections.max("ordenpresentacion"));

...
public class ParametrosmastDaoImpl extends GenericoDaoImpl<BeanParametrosmast, BeanParametrosmastPk> {
	...
	public String obtenerparametroTexto(String aplicacioncodigo,String parametroclave) {
		DtoComunParametrosmast dto = null;
		List<DominioParametroPersistencia> parametros = new ArrayList<DominioParametroPersistencia>();
		
		parametros.add(new DominioParametroPersistencia("p_aplicacioncodigo", String.class, aplicacioncodigo));
		parametros.add(new DominioParametroPersistencia("p_parametroclave", String.class, parametroclave));
		
		List lst = listarPorQuery(DtoComunParametrosmast.class, "parametrosmast.obtenerparametro", parametros);
		if (lst.size() == 1) {
			dto = (DtoComunParametrosmast) lst.get(0);
			return dto.getTexto();
		}else {

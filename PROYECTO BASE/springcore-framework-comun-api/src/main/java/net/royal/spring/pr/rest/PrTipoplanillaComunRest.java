package net.royal.spring.pr.rest;
...
//	}
	 ...
	 * @return
	 */
	@ApiOperation(notes = "-Descripcion: Listado de Planillas activas de la tabla PR_TipoPlanilla | Sin parametros de entrada | Parametros de salida: codigo, nombre", value = "PR-TIPPLAN-CLIACT", tags = {
			"PLANILLA", "PLANILLAS" })
	@Transactional
	...
	public ResponseEntity<List<DtoTabla>> listarActivosPorSeguridad(@RequestBody DtoTabla filtro) {
		...
		parametros.add(new DominioParametroPersistencia("p_tipoplanilla", String.class, filtro.getNombre()));
		parametros.add(new DominioParametroPersistencia("p_allTiposdePlanilla", String.class, filtro.getCodigo()));

		List datos = this.listarPorQuery(DtoTabla.class, "prtipoplanilla.listarActivosPorSeguridad", parametros);
		return new ResponseEntity<List<DtoTabla>>(datos, HttpStatus.OK);
	}

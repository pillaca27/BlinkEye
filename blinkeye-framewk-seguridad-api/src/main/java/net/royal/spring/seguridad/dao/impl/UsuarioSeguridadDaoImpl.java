...
@Repository
public class UsuarioSeguridadDaoImpl extends GenericoDaoImpl<Usuario, UsuarioPk> {

	private static Logger logger = LogManager.getLogger(Usuario.class);

	public UsuarioSeguridadDaoImpl() {
		super("usuario");
	}
	...
	/* NUEVO MENU CORE */
	public List<DtoSeguridadPerfilUsuario> listarDtoPerfilUsuarioPorAplicacionUsuario(String aplicacionId,String usuarioId) throws Exception {
		List<DominioParametroPersistencia> parameters = new ArrayList<DominioParametroPersistencia>();
		...
		List lista = listarPorQuery(DtoSeguridadPerfilUsuario.class, "usuario.listarDtoPerfilUsuarioPorAplicacionUsuario", parameters);
		return lista;
	}

	public SeguridadUsuarioActual loginPostulante(String tipodocumento, String documento, String clave) {
		List<DominioParametroPersistencia> parametros = new ArrayList<DominioParametroPersistencia>();
		parametros.add(new DominioParametroPersistencia("p_tipodocumento", String.class, tipodocumento));
		parametros.add(new DominioParametroPersistencia("p_documento", String.class, documento));
		parametros.add(new DominioParametroPersistencia("p_clave", String.class, clave));
		List lst = listarPorQuery(SeguridadUsuarioActual.class, "usuario.loginPostulante", parametros);
		if (lst.size() == 1) {
	...
	public String obtenerFoto(String companiaCodigo, Integer personaId, String tipoUsuarioId) {
		List<DominioParametroPersistencia> parametros = new ArrayList<DominioParametroPersistencia>();
		parametros.add(new DominioParametroPersistencia("p_tipoUsuarioId", String.class, tipoUsuarioId));
		parametros.add(new DominioParametroPersistencia("p_compania", String.class, companiaCodigo));
		parametros.add(new DominioParametroPersistencia("p_empleado", Integer.class, personaId));
		List lst = listarPorQuery(DtoTabla.class, "usuario.obtenerFotoActual", parametros);
		if (lst.size() == 0) {
			return "";
		}
	...
	public List armarArbol(List<SeguridadMenuItem> lstEncontrados, List<SeguridadMenuItem> lstGrupos,
		...
		if (UString.esNuloVacio(mnuGrp1.getAuxContenedorPadreId())) {
			...
			SeguridadMenuItem mnuGrpPadre = lstGrupos.stream().filter(u -> u.getId().equals(id)).findFirst()
					.orElse(null);
			if (mnuGrpPadre == null) {
		...
		DtoSegAutorizacion mnuGrp2 = lstGruposTodos.stream()
				...
				.filter(u -> u.getContenedorId().equals(mnuGrp1.getAuxContenedorPadreId())).findFirst().orElse(null);
		...
		if (!UString.esNuloVacio(mnuGrp2.getAuxContenedorPadreId())) {
			...
			DtoSegAutorizacion mnuGrp3 = lstGruposTodos.stream()
					...
					.filter(u -> u.getContenedorId().equals(mnuGrp2.getAuxContenedorPadreId())).findFirst()
					.orElse(null);
			...
			if (!UString.esNuloVacio(mnuGrp3.getAuxContenedorPadreId())) {
				...
				DtoSegAutorizacion mnuGrp4 = lstGruposTodos.stream()
						...
						.filter(u -> u.getContenedorId().equals(mnuGrp3.getAuxContenedorPadreId())).findFirst()
						.orElse(null);
				...
				if (!UString.esNuloVacio(mnuGrp4.getAuxContenedorPadreId())) {
					...
					DtoSegAutorizacion mnuGrp5 = lstGruposTodos.stream()
							...
							.filter(u -> u.getContenedorId().equals(mnuGrp4.getAuxContenedorPadreId())).findFirst()
							.orElse(null);
		...
		for (SeguridadMenuItem o : lstTemporal) {
			if (index == 0) {
				nivel1 = lstGrupos.stream().filter(u -> u.getId().equals(o.getId())).findFirst().orElse(null);
				if (nivel1 == null) {
			...
			if (index == 1) {
				nivel2 = nivel1.getItems().stream().filter(u -> u.getId().equals(o.getId())).findFirst().orElse(null);
				if (nivel2 == null) {
			...
			if (index == 2) {
				nivel3 = nivel2.getItems().stream().filter(u -> u.getId().equals(o.getId())).findFirst().orElse(null);
				if (nivel3 == null) {
			...
			if (index == 3) {
				nivel4 = nivel3.getItems().stream().filter(u -> u.getId().equals(o.getId())).findFirst().orElse(null);
				if (nivel4 == null) {
	...
	public List<DtoSegAutorizacion> listarDtoAutorizacionesDuplicadasPorUsuarioYPerfil(String aplicacion,String usuario) throws Exception {
		...
		parametros.add(new DominioParametroPersistencia("p_aplicacionid", String.class, aplicacion));
		parametros.add(new DominioParametroPersistencia("p_usuarioid", String.class, usuario));
		List datos = this.listarPorQuery(DtoSegAutorizacion.class, "usuario.listarDtoAutorizacionesDuplicadasPorUsuarioYPerfil", parametros);
		return datos;
	}

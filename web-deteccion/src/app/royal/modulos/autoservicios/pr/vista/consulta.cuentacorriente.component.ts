...
@Component({
  templateUrl: './consulta.cuentacorriente.component.html'
})
export class ConsultaCuentaCorrienteComponent extends FormularioComponent implements OnInit, UIListadoController {
  ...
  ngOnInit() {
    ...
    this.bloquearPagina();
    this.prPlanillaempleadoServicio.consultaPrestamos().then(res => {
      this.desbloquearPagina();
      this.lstCabecera = res;
    });
  }

  cargarDetalle(bean: any) {
    this.lstDetalle = [];
    this.bloquearPagina();
    this.prPlanillaempleadoServicio.consultaCuentaCorriente(bean)
      .then(res => {
        this.desbloquearPagina();
        this.lstDetalle = res;
      });

...
@Component({
    templateUrl: './postulante-cursos.component.html',
    selector: 'postulante-cursos'
})
export class PostulanteCursosComponent extends FormularioComponent implements OnInit {
    ...
    cargarCombos(): Promise<number>[] {
        ...
        var p = [];
        p.push(this.hrActualizacionfichaempleadoServicio.listarCursosActivos("C").then(respuesta => {
    ...
    refrescarVista(): Promise<number> {
        return this.hrPostulanteService.cursosListar(this.postulante).then(res => {
            this.lista = res;
            return 1;
        });
    ...
    eliminar(dto: any): void {
        this.confirmationService.confirm({
            ...
            accept: () => {
                this.bloquearPagina();
                this.hrPostulanteService.crud(dto, 'cursoEliminar').then(res => {
                    this.desbloquearPagina();
    ...
    guardar() {
        ...
        var accionCRUD = this.accion == this.ACCIONES.NUEVO ? 'cursoRegistrar' : 'cursoActualizar';

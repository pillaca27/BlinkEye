...
@Component({
    templateUrl: './postulante-informatica.component.html',
    selector: 'postulante-informatica'
})
export class PostulanteInformaticaComponent extends FormularioComponent implements OnInit {
    ...
    cargarCombos(): Promise<number>[] {
        ...
        var p = [];
        p.push(this.hrActualizacionfichaempleadoServicio.listarCursosActivos("F").then(respuesta => {
    ...
    refrescarVista(): Promise<number> {
        return this.hrPostulanteService.informaticaListar(this.postulante).then(res => {
            this.lista = res;
            return 1;
        });
    }
    ...
    eliminar(dto: any): void {
        this.confirmationService.confirm({
            ...
            accept: () => {
                this.bloquearPagina();
                this.hrPostulanteService.crud(dto, 'informaticaEliminar').then(res => {
                    this.desbloquearPagina();
    ...
    guardar() {
        ...
        var accionCRUD = this.accion == this.ACCIONES.NUEVO ? 'informaticaRegistrar' : 'informaticaActualizar';

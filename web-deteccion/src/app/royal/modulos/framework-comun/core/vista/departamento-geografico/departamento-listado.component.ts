...
import { FiltroComunDepartamento } from '@framework-comun/core/dominio/filtro/FiltroComunDepartamento';
...
export class DepartamentoListadoComponent extends FormularioComponent implements OnInit, UIListadoController {

    @ViewChild(BotonesListadoComponent, { static: false }) botonesComponent: BotonesListadoComponent;
    @ViewChild(Table, { static: false }) dataTableComponent: Table;

    lstEstados: SelectItem[] = [];
    filtro: FiltroComunDepartamento = new FiltroComunDepartamento();
    blockSpecial: RegExp = /^[a-zA-Z]*$/;
    ...
    grillaCargarDatos(event: LazyLoadEvent) {
        ...
        this.bloquearPagina();
        this.departamentoService.listarpaginado(this.filtro).then((res) => {
    ...
    coreSalir() {
        if (this.esObjetoVacio(this.registroSeleccionado)) {
            this.mostrarMensajeAdvertencia('Debe seleccionar un Registro')
             return }
        this.coreDistritos(this.registroSeleccionado)
    }
    coreNuevo(): void {
        this.router.navigate([ConstanteComun.ruta_departamento_mantenimiento, this.ACCIONES.NUEVO], { skipLocationChange: true })
    }

    coreEditar(dto: DtoComunDepartamento) {
        dto.accion = "PROV"
        this.router.navigate([ConstanteComun.ruta_departamento_mantenimiento, this.ACCIONES.EDITAR, JSON.stringify(dto)], { skipLocationChange: true });
    }

    coreVer(dto: DtoComunDepartamento) {
        dto.accion = "PROV"
        this.router.navigate([ConstanteComun.ruta_departamento_mantenimiento, this.ACCIONES.VER, JSON.stringify(dto)], { skipLocationChange: true });
    }
    coreDistritos(dto: DtoComunDepartamento) {
        if (this.estaVacio(dto.pais)) {
            dto.accion = ""
            this.mostrarMensajeAdvertencia("El departamento no tiene provincia.")
            return
        }
        dto.accion = "DISTR"
        this.router.navigate([ConstanteComun.ruta_departamento_mantenimiento, this.ACCIONES.EDITAR, JSON.stringify(dto)], { skipLocationChange: true });
    ...
    coreEliminar(dto: DtoComunDepartamento) {
        dto.accion = "DELETE"
        this.router.navigate([ConstanteComun.ruta_departamento_mantenimiento, this.ACCIONES.ELIMINAR, JSON.stringify(dto)], { skipLocationChange: true });
    }
    ...
    exportar() {
        ...
        this.bloquearPagina();
        this.departamentoService.exportarDepartamentosGeograficos(this.filtro)
            .subscribe(
                res => {
                    this.desbloquearPagina();
                    this.obtenerDescarga(res, 'Listado de Departamentos Geográficos', this.modificarFormatoFechaExport(new Date))
                }, error => {
                    throw error;
                }, () => {

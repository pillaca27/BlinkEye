...
import { FiltroComunBanco } from '@framework-comun/core/dominio/filtro/FiltroComunBanco';
...
export class BancoListadoComponent extends FormularioComponent implements OnInit, UIListadoController {

    @ViewChild(BotonesListadoComponent, { static: false }) botonesComponent: BotonesListadoComponent;
    @ViewChild(Table, { static: false }) dataTableComponent: Table;

    lstEstados: SelectItem[] = [];
    filtro: FiltroComunBanco = new FiltroComunBanco();
    ...
    grillaCargarDatos(event: LazyLoadEvent) {
        ...
        this.bloquearPagina();
        this.bancoService.listarpaginado(this.filtro).then((res) => {
    ...
    coreNuevo(): void {
        this.router.navigate([ConstanteComun.ruta_banco_mantenimiento, this.ACCIONES.NUEVO], { skipLocationChange: true })
    }

    coreEditar(dto: DtoComunBanco) {
        this.router.navigate([ConstanteComun.ruta_banco_mantenimiento, this.ACCIONES.EDITAR, JSON.stringify(dto)], { skipLocationChange: true });
    }

    coreVer(dto: DtoComunBanco) {
        this.router.navigate([ConstanteComun.ruta_banco_mantenimiento, this.ACCIONES.VER, JSON.stringify(dto)], { skipLocationChange: true });
    }

    coreAnular(dto: DtoComunBanco) {
        this.confirmationService.confirm({
            header: "Confirmación", icon: "fa fa-question-circle", message: "¿Desea anular este registro ? ",
            accept: () => {
                this.bloquearPagina();
                this.bancoService.anular(dto).then((res) => {
                    this.desbloquearPagina();
    ...
    coreEliminar(dto: DtoComunBanco) {
        this.router.navigate([ConstanteComun.ruta_banco_mantenimiento, this.ACCIONES.ELIMINAR, JSON.stringify(dto)], { skipLocationChange: true });
    }
    ...
    exportar() {
        ...
        this.bloquearPagina();
        this.bancoService.exportarBancos(this.filtro)
            .subscribe(
                res => {
                    this.desbloquearPagina();
                    this.obtenerDescarga(res, 'Listado de Bancos', this.modificarFormatoFechaExport(new Date))
                }, error => {
                    throw error;
                }, () => {

...
@Component({
    selector: 'app-syunidadreplicacion-listado',
    templateUrl: './syunidadreplicacion-listado.component.html',
})
export class SyUnidadreplicacionListadoComponent extends FormularioComponent implements OnInit, UIListadoController {

    @ViewChild(BotonesListadoComponent, { static: false }) botonesComponent: BotonesListadoComponent;
    @ViewChild(Table, { static: false }) dataTableComponent: Table;

    lstEstados: SelectItem[] = [];
    filtro: FiltroComunSyUnidadreplicacion = new FiltroComunSyUnidadreplicacion();
    ...
    grillaCargarDatos(event: LazyLoadEvent) {
        ...
        this.bloquearPagina();
        this.syUnidadreplicacionService.listarpaginado(this.filtro).then((res) => {
    ...
    coreNuevo(): void {
        this.router.navigate([ConstanteComun.ruta_syunidadreplicacion_mantenimiento, this.ACCIONES.NUEVO], { skipLocationChange: true })
    }

    coreEditar(dto: DtoComunSyUnidadreplicacion){
        this.router.navigate([ConstanteComun.ruta_syunidadreplicacion_mantenimiento, this.ACCIONES.EDITAR, JSON.stringify(dto)], { skipLocationChange: true });
    }

    coreVer(dto: DtoComunSyUnidadreplicacion){
        this.router.navigate([ConstanteComun.ruta_syunidadreplicacion_mantenimiento, this.ACCIONES.VER, JSON.stringify(dto)], { skipLocationChange: true });
    }

    coreAnular(dto: DtoComunSyUnidadreplicacion) {
        this.confirmationService.confirm({
            header: "Confirmación",icon: "fa fa-question-circle",message: "¿Desea anular este registro ? ",
            accept: () => {
            this.bloquearPagina();
            this.syUnidadreplicacionService.anular(dto).then((res) => {
            this.desbloquearPagina();
    ...
    coreEliminar(dto: DtoComunSyUnidadreplicacion) {
        this.router.navigate([ConstanteComun.ruta_syunidadreplicacion_mantenimiento, this.ACCIONES.ELIMINAR, JSON.stringify(dto)], { skipLocationChange: true });
    }
    ...
    exportar() {
        ...
        this.bloquearPagina();
        this.syUnidadreplicacionService.exportarUnidadReplicacion(this.filtro)
            .subscribe(
                res => {
                    this.desbloquearPagina();
                    this.obtenerDescarga(res, 'Listado de Unidad de Replicación', this.modificarFormatoFechaExport(new Date))
                }, error => {
                    throw error;
                }, () => {
